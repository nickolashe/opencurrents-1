...
class PublicRecordViewTestSuite(TestCase):
    ...
    def set_up_org(self, status, old=False):
        org = OcOrg().setup_org(name=self._generate_test_name(), status=status)
        user_org = OcUser().setup_user(
            username=self._generate_test_name(),
            email=self._generate_test_name(),
        )
        ...
        event = Event.objects.create(
            project=project,
            description=self._generate_test_name(),
            location=self._generate_test_name(),
            coordinator=user_org,
            creator_id=user_org.id,
            event_type=self._generate_test_name(),
        ...
        if old:
            ...
            t.date_created = datetime.now() - timedelta(days=33)
            t.date_updated = datetime.now() - timedelta(days=33)
            t.save()
